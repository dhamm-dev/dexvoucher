...
const Products = {
  ...
  renderGamePage(gameId) {
    ...
    if (!game) {
      document.body.innerHTML = '<div class="container" style="padding:100px 20px;text-align:center"><h1>Game tidak ditemukan</h1><a href="' + ROOT + '/index.html" class="btn btn-primary mt-lg">Kembali ke Home</a></div>';
      return;
    }
      ...
      formContainer.innerHTML = `
        <div class="card">
          ...
          <div style="margin-top:12px">
            <button class="btn btn-outline btn-sm" id="check-id-btn" type="button"><img class="icon-img" src="${ROOT}/assets/images/icons/search.png" alt="" style="width:14px;height:14px"> Cek ${accountLabel}</button>
  ...
  _renderProducts(gameId, category) {
    ...
    if (filtered.length === 0) {
      grid.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><div class="empty-icon"><img class="icon-img" src="' + ROOT + '/assets/images/icons/Product.png" alt="" style="width:36px;height:36px"></div><h3>Tidak ada produk</h3><p>Belum ada produk untuk kategori ini</p></div>';
      return;
    }
  ...
  _bindEvents(gameId, game) {
    ...
    if (checkBtn) {
      checkBtn.addEventListener('click', function () {
        ...
        result.innerHTML = '<img class="icon-img" src="' + ROOT + '/assets/images/icons/sukses.png" alt="" style="width:16px;height:16px;vertical-align:middle"> ' + acctLabel + ' valid';
    ...
    document.querySelectorAll('.btn-cart').forEach(btn => {
      btn.addEventListener('click', function () {
        ...
        if (!session) {
          window.location.href = ROOT + '/pages/user/login.html?redirect=' + encodeURIComponent(window.location.pathname);
          return;
        }
        const serverId = document.getElementById('server-id');
  ...
  _showConfirmModal(product, accountId, serverId) {
    ...
    document.getElementById('modal-confirm').addEventListener('click', function () {
      ...
      window.location.href = ROOT + '/pages/user/checkout.html';
    });
  },
